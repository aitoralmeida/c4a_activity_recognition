...
WINDOW_SIZE = 4
# Window distance penalization
WINDOW_PENALIZATION = 0.30
# Window distance penalization threshold
WINDOW_PENALIZATION_THRESHOLD = 0.05
# Minimum difference
...
def calculate_context_similarity(actions, embedding_action_matrix, position, window):
    ...
    for i in range(1, window+1):
        ...
        if position+i < len(actions):
            ...
            target_action_vector_context_sim += right_sim * max(WINDOW_PENALIZATION_THRESHOLD, 1 - (WINDOW_PENALIZATION * (i-1)))
        else:
            counter -= 1
        ...
        if position-i >= 0:
            ...
            target_action_vector_context_sim += left_sim * max(WINDOW_PENALIZATION_THRESHOLD, 1 - (WINDOW_PENALIZATION * (i-1)))
        else:
            counter -= 1
