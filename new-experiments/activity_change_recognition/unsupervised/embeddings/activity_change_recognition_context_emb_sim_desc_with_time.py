...
# Max time (for example, 24 h daily activity) in seconds
MAX_TIME = 24 * 60 * 60

def get_unixtime(dt64):
    return dt64.astype('datetime64[s]').astype('int')

def get_time_similarity(time_a, time_b):
    return max(0, 1 - (abs(time_b - time_a) / MAX_TIME)) # USMART time similarity paper

def prepare_x_y_activity_change(df):
    actions = df['action'].values
    timestamps = list(df.index.values)
    timestamps = list(map(get_unixtime, timestamps))
    ...
    return X, timestamps, y, tokenizer_action
...
def calculate_context_similarity_with_time(actions, timestamps, embedding_action_matrix, position, window):
    target_action_vector = embedding_action_matrix[actions[position]]
    target_action_vector_context_sim = 0.0
    counter = window * 4
    for i in range(1, window+1):
        ...
        if position+i < len(actions):
            right_sim = 1 - spatial.distance.cosine(target_action_vector, embedding_action_matrix[actions[position+i]])
            right_sim_time = get_time_similarity(timestamps[position], timestamps[position+i])
            target_action_vector_context_sim += right_sim
            target_action_vector_context_sim += right_sim_time
        else:
            counter -= 2
        # left context we search for disimilarity (1 - similarity)
        if position-i >= 0:
            left_sim = 1 - max(0, 1 - spatial.distance.cosine(target_action_vector, embedding_action_matrix[actions[position-i]]))
            left_sim_time = max(0, 1 - get_time_similarity(timestamps[position-i], timestamps[position]))
            target_action_vector_context_sim += left_sim
            target_action_vector_context_sim += left_sim_time
        else:
            counter -= 2
    target_action_vector_context_sim = target_action_vector_context_sim / counter
    return target_action_vector_context_sim
...
def main(argv):
    ...
    print("### ### ### ###")
    # prepare dataset
    X, timestamps, y, tokenizer_action = prepare_x_y_activity_change(df_dataset)
    ...
    for i in range(0, len(X)):
        context_similarity = calculate_context_similarity_with_time(X, timestamps, embedding_action_matrix, i, WINDOW_SIZE)
