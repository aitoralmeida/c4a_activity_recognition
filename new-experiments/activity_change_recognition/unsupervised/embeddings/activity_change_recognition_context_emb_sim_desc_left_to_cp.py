...
#TODO IDEA: LEFT CONTEXT EN LUGAR DE WINDOW_SIZE HASTA EL ANTERIOR CHANGE POINT.
def calculate_context_similarity(actions, embedding_action_matrix, position, last_cp_position, window):
    ...
    for pos in range(last_cp_position, position):
        if position-i >= 0:
            left_sim = 1 - max(0, 1 - spatial.distance.cosine(target_action_vector, embedding_action_matrix[actions[pos]]))
...
def main(argv):
    ...
    for i in range(0, len(X)):
        context_similarity = calculate_context_similarity(X, embedding_action_matrix, i, last_change_point_pos, WINDOW_SIZE)
        # print("Context similarity: " + str(context_similarity) + " Activity change: " + str(y[i]))
        similarities.append(context_similarity)
        if i != 0:
            predicted_label = 1 if (similarities[i] - similarities[i-1] > REQUIRED_DIFFERENCE) else 0
            y_pred.append(predicted_label)
            if (predicted_label == 1):
                discovered_activity_num += 1
            discovered_activities.append('ACT' + str(discovered_activity_num))
            last_change_point_pos = i
