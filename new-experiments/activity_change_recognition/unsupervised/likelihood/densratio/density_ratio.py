...
class DensityRatio:
    """Density Ratio."""
    def __init__(self, method, alpha, theta, lambda_, alpha_PE, alpha_KL, alpha_SEP, kernel_info, compute_density_ratio):
        self.method = method
        self.alpha = alpha
        self.theta = theta
...
"""[1:-1] % dict(method=self.method, kernel_info=self.kernel_info, alpha=self.alpha, theta=my_format(self.theta), lambda_=self.lambda_, alpha_PE=self.alpha_PE, alpha_KL=self.alpha_KL, alpha_SEP=self.alpha_SEP)
