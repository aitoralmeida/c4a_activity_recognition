...
def RuLSIF(x, y, alpha, sigma_range, lambda_range, kernel_num=100, verbose=False):
    ...
    # Compute the approximate alpha-relative SEP distance, given samples x and y from the respective distributions.
    def alpha_SEP_distance(x, y):
        # This is Y, in Reference 1.
        x = to_numpy_matrix(x)
        ...
        # Compute the alpha-relative SEP distance
        n = x.shape[0]
        return max(0, ((1/2) * ((1/n)*g_x.sum(axis=0))))

    alpha_PE = alpha_PE_divergence(x, y)
    alpha_KL = alpha_KL_divergence(x, y)
    alpha_SEP = alpha_SEP_distance(x, y)
    ...
    result = DensityRatio(method="RuLSIF", alpha=alpha, theta=theta, lambda_=lambda_, alpha_PE=alpha_PE, alpha_KL=alpha_KL, alpha_SEP=alpha_SEP,
